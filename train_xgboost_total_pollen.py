...
def select_features(df):
    ...
    from pollen_intensity_predictor import select_features as baseline_select
...
def main():
    ...
    # Save
    print(f"\n💾 Saving outputs...")
    joblib.dump(model, 'pollen_xgboost_total_pollen_optimized.joblib')
    ...
    with open('xgboost_total_pollen_features.json', 'w') as f:
        json.dump({k: float(v) for k, v in feature_importance.items()}, f, indent=2)
    print(f"   ✅ Features: xgboost_total_pollen_features.json")
    
    # Save results
    results['predictions'] = [float(x) for x in y_pred]
    results['actuals'] = [float(x) for x in y_true]
    
    with open('xgboost_total_pollen_results.json', 'w') as f:
    ...
    plt.title('XGBoost Total_Pollen: Top 20 Features')
    plt.gca().invert_yaxis()
    plt.tight_layout()
    plt.savefig('xgboost_total_pollen_importance.png', dpi=150, bbox_inches='tight')
    print(f"   ✅ Plot: xgboost_total_pollen_importance.png")
    
    # Prediction scatter plot
    plt.figure(figsize=(8, 8))
    plt.scatter(y_true, y_pred, alpha=0.5, s=30)
    plt.plot([0, 10], [0, 10], 'r--', lw=2, label='Perfect Prediction')
    plt.xlabel('Actual Severity (0-10)')
    plt.ylabel('Predicted Severity (0-10)')
    plt.title(f'XGBoost Total_Pollen\nMAE: {results["mae"]:.3f}, R²: {results["r2"]:.3f}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.xlim(0, 10)
    plt.ylim(0, 10)
    plt.tight_layout()
    plt.savefig('xgboost_total_pollen_predictions.png', dpi=150, bbox_inches='tight')
    print(f"   ✅ Plot: xgboost_total_pollen_predictions.png")
